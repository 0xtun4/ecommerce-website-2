$(document).ready(function(){
    ...
    let roomID = CryptoJS.MD5("id"+ sessionStorage.getItem("id") + "qwerty");
    sessionStorage.setItem("room", roomID);
    const socket = io(DOMEN_SERVER, {
        transports : ['websocket', 'polling', 'flashsocket'],
        query : 'id='+sessionStorage.getItem("id")+"&room="+sessionStorage.getItem("room")  });
    ...
    const addChatMessage = (data, options = {}) => {
        var div;
        if (data.adm != 0) {
            div = '<li class="clearfix" data-mid="'+data.label+'">' +
                '<div class="message-data "><img src="'+ DOMEN +'css/piccl.png">' +
                '<span class="message-data-time">'+ data.adm + ' ' + data.date + '</span>' +
                '</div>' +
                '<div class="message my-message">' + data.message +'</div></li>';
        }  else {
            div = '<li class="clearfix" data-mid="'+data.label+'">' +
                '<div class="message-data text-right">' +
                '<span class="message-data-time">id' + data.username + ' ' + data.date + '</span>' +
                '<img src="'+ DOMEN +'css/techhelp.png"></div>' +
    ...
    const addChatMessageHistory = (data, options = {}) => {
        ...
        if (data.adm != 0) {
            div = '<li class="clearfix" data-mid="'+data.label+'">' +
                '<div class="message-data "><img src="'+ DOMEN +'css/piccl.png">' +
                '<span class="message-data-time">'+ data.adm + ' ' + data.date + '</span></div>' +
                '<div class="message my-message">' + data.message +'</div></li>';
        }  else {
            div = '<li class="clearfix" data-mid="'+data.label+'">' +
                '<div class="message-data text-right">' +
